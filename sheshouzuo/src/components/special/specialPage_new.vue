    ...
    <Row type="flex">
      <i-col span="22" offset="1">
        <Tabs value="name1" class="demo-tabs-style1" :animated="animated">
          <TabPane label="新建专题页" name="name1">
            ...
            <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
              ...
              <FormItem label="专题页封面" prop="cover">
                ...
                <input type="hidden" v-model="formValidate.cover">
              </FormItem>
    ...
    methods: {
      ...
      handleSubmit(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            var that=this;
            this.$ajax.post("/special/specialPage_new_ajax", {
              title: this.formValidate.title,
              cover: this.formValidate.cover,
              content:this.htmlForEditor,
              goodsInfo:this.targetKeys2 ,
              isReward: this.formValidate.isReward,
              shareGroupAmount: this.formValidate.shareGroupAmount,
              shareGroupReward: this.formValidate.shareGroupReward,
              shareFriendReward: this.formValidate.shareFriendReward,
              fullReward: this.formValidate.fullReward,
            }).then(function (res) {
              if(res.data.error=='0'){
                that.$Message.success('专题页添加成功!');
                setTimeout(function () {
                  location.reload()
                },400)
